...
met_67 = abjad.MetronomeMark((1, 4), 67)
...
met_90 = abjad.MetronomeMark((1, 4), 90) # mean between 100 and 80 for accel / rall

met_79 = abjad.MetronomeMark((1, 4), 79) # mean between 91 and 67 for accel / rall

# markup met

met_67_mark = abjad.MetronomeMark.make_tempo_equation_markup((1, 4), 67)

met_80_mark = abjad.MetronomeMark.make_tempo_equation_markup((1, 4), 80)
...
mark_67_small = abjad.LilyPondLiteral(
    [
        r"^ \markup {",
        r"  \with-dimensions-from \null",
...
mark_80_small = abjad.LilyPondLiteral(
    [
        r"^ \markup {",
        r"  \with-dimensions-from \null",
        r"  \override #'(font-size . 1)",
...
mark_100_small = abjad.LilyPondLiteral(
    [
        r"^ \markup {",
        r"  \with-dimensions-from \null",
        # r"  \override #'(font-size . 5.5)", # score
        r"  \override #'(font-size . 1)",  # parts
        r'''  \override #'(font-name . "Helvetica Bold")''',
        r"  \concat {",
        f"      {met_100_mark.string[8:]}",
        r"  }",
...
def add_time_marks(selections):
    measures = abjad.select.group_by_measure(selections)
    for measure in measures:
        last_leaf = abjad.select.leaf(measure, -1)
        abjad.attach(abjad.LilyPondLiteral(r"% \timeMark", site="absolute_after"), last_leaf)


def integrate_multi_trills(selections, intervals=["+P4"], harmonics_boolean_vector=[1, 0]):
    cyc_harmonics = evans.CyclicList(harmonics_boolean_vector, forget=False)
    cyc_intervals = evans.CyclicList(intervals, forget=False)
    ties = abjad.select.logical_ties(selections, pitched=True)
    for tie in ties:
        local_interval = cyc_intervals(r=1)[0]
        if local_interval is not None:
            if not isinstance(local_interval, list):
            ...
            abjad.attach(
                abjad.bundle(
                    ...
                    rf"- \tweak details.n-copies {len(local_interval)}",
            ...
            local_pitch = tie[0].written_pitch
            named_intervals = []
            if cyc_harmonics(r=1)[0] == 1:
                harmonic_value = True
            else:
                harmonic_value = False
            if harmonic_value is True:
                for next_interval in local_interval:
                    if abjad.NamedInterval(next_interval).name[0] == "+":
                        named_intervals.append(abjad.NamedInterval(next_interval))
                    elif abjad.NamedInterval(next_interval).name[0] == "-":
                        named_intervals.append(abjad.NamedInterval(next_interval) + abjad.NamedInterval("+P8"))
            else:
                for next_interval in local_interval:
                    named_intervals.append(abjad.NamedInterval(next_interval))
            calculated_pitches = [current_interval.transpose(local_pitch) for current_interval in named_intervals]
            abjad.attach(
                evans.AfterGraceContainer(
                    [abjad.Note(calculated_pitch, (1, 4)) for calculated_pitch in calculated_pitches],
                    as_trill_heads=True,
                    harmonics=harmonic_value,
                ),
                tie[0]
            )
